...
function Write-Status {
	...
	$statusChar = if ($Level -eq [LogLevel]::Success -or $Level -eq [LogLevel]::Info) { '+' } else { '-' }

	$Text -split "`n" | ForEach-Object {
		Write-Log $statusChar "$_"
	}
...
Error: $_" -Level Critical -Exit -ExitCode 5
	...
	Write-Host "`nDownloading Microsoft Edge:"
	@(
...
if (!$UninstallEdge -and !$InstallEdge -and !$InstallWebView) {
	$host.UI.RawUI.WindowTitle = "EdgeRemover $version | made by LexBoosT"
...
"@
		...
		Write-Host "`nEdge is currently detected as: " -NoNewline
		Write-Host "$(@("Uninstalled", "Installed")[$edgeInstalled])"

		Write-Host "`n$("-" * $description.Length)"

		if ($RemoveEdgeData) {$colourData = "Green"; $textData = "Selected"} else {$colourData = "Red"; $textData = "Unselected"}

		Write-Host "`nOptions:"
		Write-Host "[1] Remove Edge User Data ($textData)"
